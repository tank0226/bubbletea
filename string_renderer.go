...
// stringRenderer is a timer-based renderer, updating the view at a given framerate
...
func newStringRenderer(out io.Writer, mtx *sync.Mutex) *stringRenderer {
...
func (r *stringRenderer) stop() {
	r.flush()
	r.done <- struct{}{}
}

// listen waits for ticks on the ticker, or a signal to stop the renderer.
func (r *stringRenderer) listen() {
	for {
		select {
		case <-r.ticker.C:
			if r.ticker != nil {
...
func (r *stringRenderer) write(s string) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
	r.buf.Reset()
...
// renderer.
func (r *stringRenderer) setIgnoredLines(from int, to int) {
...
// rendered to again.
func (r *stringRenderer) clearIgnoredLines() {
	r.ignoreLines = nil
}
...
func (r *stringRenderer) insertTop(lines []string, topBoundary, bottomBoundary int) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
...
func (r *stringRenderer) insertBottom(lines []string, topBoundary, bottomBoundary int) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
...
func (r *stringRenderer) handleMessages(msg Msg) {
	switch msg := msg.(type) {
	case WindowSizeMsg:
		r.width = msg.Width
		r.height = msg.Height
